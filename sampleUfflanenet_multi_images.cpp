...
void* createLaneNetCudaBuffer(int64_t eltCount, DataType dtype, int run, vector<string> test_image_list)
{
    ...
    string reading_image = test_image_list[run];
    cout << reading_image << endl;

    //cv::Mat rgbImage = cv::imread("/home/kai/Desktop/TensorRT-4.0.1.6/data/lanenet/0000.png");
    cv::Mat rgbImage = cv::imread(reading_image);
    cv::Mat image;
...
void getOutput_instance_seg(int64_t eltCount, DataType dtype, void* buffer, int run, string instance_image_dir)
{
    ...
    */
    ...
    string sss = to_string(run);
    instance_image_dir = instance_image_dir.append(sss);
    instance_image_dir = instance_image_dir.append(".jpg");
    cout << instance_image_dir << endl;
    /*
    cv::imshow("back_image", back_image);
    cv::waitKey(0);
    */
    cv::imwrite (instance_image_dir, back_image);
    
    delete[] outputs;
}

void getOutput_binary_seg(int64_t eltCount, DataType dtype, void* buffer, int run, string binary_image_dir)
{
    //std::cout << eltCount << " eltCount" << std::endl;
    assert(elementSize(dtype) == sizeof(float));
    //std::cout << "--- OUTPUT ---" << std::endl;
    ...
    */
    ...
    string sss = to_string(run);
    binary_image_dir = binary_image_dir.append(sss);
    binary_image_dir = binary_image_dir.append(".jpg");
    cout << binary_image_dir << endl;
    /*
    cv::imshow("final_image", back_image);
    cv::waitKey(0);
    */
    cv::imwrite (binary_image_dir, back_image);    
...
void execute(ICudaEngine& engine)
{
    ...
    int iterations = 1;
    int numberRun = 14504; // num of image
    char line[100];
    // read test image list
    fstream rtil;
    rtil.open("/home/kai/Desktop/TensorRT-4.0.1.6/data/lanenet/test_image_list.txt", ios::in);
    vector<string> test_image_list;
    while(rtil.getline(line,sizeof(line),'\n'))
    {
        string tmp_string(line);
        test_image_list.push_back(tmp_string);
    }

    string instance_image_dir = "/home/kai/Desktop/TensorRT-4.0.1.6/data/lanenet/instance_image/";
    string binary_image_dir = "/home/kai/Desktop/TensorRT-4.0.1.6/data/lanenet/binary_image/";
    ...
    for (int i = 0; i < iterations; i++)
    {
        ...
        for (int run = 0; run < numberRun; run++)
        {
            buffers[bindingIdxInput] = createLaneNetCudaBuffer(bufferSizesInput.first,
                                                             bufferSizesInput.second, run, test_image_list);
            ...
            for (int bindingIdx = 0; bindingIdx < nbBindings; ++bindingIdx)
            {
                ...
                if (bindingIdx == 1){
                    ...
                    getOutput_instance_seg(bufferSizesOutput.first, bufferSizesOutput.second, buffers[bindingIdx], run, instance_image_dir);
                }

                if (bindingIdx == 2){
                    auto bufferSizesOutput = buffersSizes[bindingIdx];
                    //cout << bindingIdx << endl;
                    getOutput_binary_seg(bufferSizesOutput.first, bufferSizesOutput.second, buffers[bindingIdx], run, binary_image_dir);
                }
            }
            CHECK(cudaFree(buffers[bindingIdxInput]));
        }

        float fps = numberRun/total*1000;
        total /= numberRun;
        std::cout << "Average over " << numberRun << " runs is " << total << " ms." << " fps is " << fps << std::endl;
    }
