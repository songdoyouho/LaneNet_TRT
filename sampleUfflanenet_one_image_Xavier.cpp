...
void getOutput_instance_seg(int64_t eltCount, DataType dtype, void* buffer)
{
    ...
    for(int i=0; i < back_image.rows; i++)
    {
        ...
        {
            back_image.at<cv::Vec3f>(i, j)[0] = outputs[iii+262144];
            back_image.at<cv::Vec3f>(i, j)[1] = outputs[iii+131072];
            back_image.at<cv::Vec3f>(i, j)[2] = outputs[iii];

            if(outputs[iii] < min_val_1)
                min_val_1 = outputs[iii];
            
            if(outputs[iii+131072] < min_val_2)
                min_val_2 = outputs[iii+131072];

            if(outputs[iii+262144] < min_val_3)
                min_val_3 = outputs[iii+262144];

            if(outputs[iii] > max_val_1)
                max_val_1 = outputs[iii];

            if(outputs[iii+131072] > max_val_2)
                max_val_2 = outputs[iii+131072];

            if(outputs[iii+262144] > max_val_3)
                max_val_3 = outputs[iii+262144];
...
void getOutput_binary_seg(int64_t eltCount, DataType dtype, void* buffer)
{
    ...
    for(int i=0; i < back_image.rows; i++)
    {
        for(int j=0; j < back_image.cols; j++)
        {
            if(outputs[iii] > outputs[iii+131072])
            {
